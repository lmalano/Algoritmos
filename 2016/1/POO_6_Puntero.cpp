...
Punto3::Punto3( int valorX, int valorY )
   : x( valorX ), y( valorY )//si viene un valor en x lo pnes en x, su viene en y lo pone en y
{
   // cuerpo vacío 
...
void  Punto3::desplX(int x)
{		estableceX(obtieneX()+x);
}
void  Punto3::desplY(int Y)
{		estableceY(obtieneY()+Y);
}

// La clase Circulo4 contiene el par de coordenadas x-y y el radio.
class Circulo4 : public Punto3 {//en la clase circulo, utilizamos los metodos de punto 3
...
public:
   ...
   void imprime() const;          // muestra el objeto Circulo4
	void increcirc(double); //incrementa el radio
private: 
...
Circulo4::Circulo4( int valorX, int valorY, double valorRadio )
   : Punto3( valorX, valorY )  // invoca al constructor de la clase base//llama al constructor del punto3
{
   estableceRadio( valorRadio );

} // fin del constructor Circulo4 

void Circulo4::increcirc (double m)
{
	estableceRadio(obtieneRadio()*m);
}
...
{  Circulo4 *circulo; // crea puntero a Circulo4
   ...
   circulo->desplY(2);//desplazo y en 2
   
   // despliega las coordenadas del punto
   cout << "La coordenada X es " << circulo->obtieneX() 
        << "\nLa coordenada Y es " << circulo->obtieneY()
        << "\nEl radio es " << (*circulo).obtieneRadio(); //(*circulo).obtieneRadio = circulo->obtieneRadio es lo mismo
   ...
   circulo->estableceRadio( 4.25 );   // establece el nuevo radio
	circulo->increcirc(2);				//incrementa el doble del radio
...
// desplacen el punto un dx y dy ok!
// incremente el circulo un % 	ok!
